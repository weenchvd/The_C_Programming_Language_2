		...
		if (err == FALSE) {
			...
			case 2:
				...
				printf(">> Result of the function month_day(y, yd, pm, pd):\n"
					"YEAR %d   MONTH %d (%s)   DAY %d\n", n1, month, month_name(month), day);
				break;
			default:
...
static char daytab[2][NMONTH] = {
	{0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31},
	{0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}
};
const char* pdaytab = daytab;
...
int day_of_year(int year, int month, int day)
{
	...
	if (day < 1 || day > *(pdaytab + NMONTH * leap + month)) {
		printf("-- Error: entered DAY must be from 1 to %d inclusive\n", (int) *(pdaytab + NMONTH * leap + month));
		return -1;
	}
	for (i = 1; i < month; i++) {
		day += (int) *(pdaytab + NMONTH * leap + i);
	}
	return day;
}
...
int month_day(int year, int yearday, int* pmonth, int* pday)
{
	...
	for (i = 1; yearday > (int) * (pdaytab + NMONTH * leap + i); i++) {
		yearday -= (int) * (pdaytab + NMONTH * leap + i);
	}
	*pmonth = i;
	*pday = yearday;
	return 0;
